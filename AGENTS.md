...
3. **アーキテクチャ上の情報境界（マルコフブランケット）の厳守**:
   - 開発プロセスにおいては、統括エージェントがプロジェクト全体を一括して編集・コミットすることを許可する（物理的な編集制限は撤廃）。
   - ただし、ソースコードの設計においては、各レイヤー（core, body, shell, monitor）間の論理的境界を厳守すること。レイヤー間の直接的な密結合や不正な双方向API呼び出しを禁じ、一方向の間接介入モデル（ファイル/共有メモリを介したシグナル通信など）を維持すること。
