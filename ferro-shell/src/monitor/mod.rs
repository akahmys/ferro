...
pub async fn run_monitor_daemon(
    ...
    mut sleep_tx: Option<oneshot::Sender<()>>,
    ...
    let surprise_path = memory_dir.join("surprise_history.csv");
    ...
    loop {
        tokio::select! {
            ...
            _ = interval.tick() => {
                ...
                if let Ok(true) = surprise_monitor::check_sleep_stability(&surprise_path) {
                    if let Some(tx) = sleep_tx.take() {
                        println!("[ferro-monitor] 💤 [SLEEP TRANSITION DETECTED] FEP stabilized in Sleep phase.");
                        let _ = tx.send(());
                    }
                }
